...
AGENT_ROOT = ROOT_DIR / "nexus-agents"


def locate_agent_module(agent_name: str) -> Path:
    candidate = AGENT_ROOT / agent_name / "agent.py"
    if not candidate.exists():
        raise FileNotFoundError(f"Agent module not found: {candidate}")
    return candidate


async def run_agent_function(agent_name: str, function_name: str, payload: dict) -> dict:
    path = locate_agent_module(agent_name)
    spec = importlib.util.spec_from_file_location(f"nexus_agents.{agent_name}", path)
